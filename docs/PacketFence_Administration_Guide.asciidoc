...
For Debian squeeze, starting with 3.3.3, PacketFence is now providing an deb repository.
...
Under Debian, the freeradius server is a dependencie of PacketFence, so you don't have to install it. Just be careful, the configuration directory is under /etc/freeradius.
